...
export const definition: TrackerDefinition = {
  ...
  login: {
    ...
    captcha: { type: 'image', selector: 'img', input: 'vImageCodP' },
    inputs: {
  ...
  search: {
    ...
    fields: {
      ...
      download: {
        ...
        attribute: 'href',
        filters: [
          { name: 'replace', args: ['download2.php?id=', 'download.php/'] },
      ...
      banner: {
        ...
        filters: [
          { name: 'regexp', args: 'img src=(.+?) >' },
          { name: 'replace', args: ['./pic/noposter.jpg', ''] },
        ],
      },
      date: { selector: 'td:nth-child(8) > span', attribute: 'title' },
      size: { selector: 'td:nth-child(6)' },
      seeders: {
        selector: 'td:nth-child(7)',
        filters: [{ name: 'regexp', args: '([\\d]+) seeders?' }],
      },
      leechers: {
        selector: 'td:nth-child(7)',
        filters: [{ name: 'regexp', args: '([\\d]+) leechers?' }],
      },
      grabs: {
        selector: 'td:nth-child(6)',
        filters: [{ name: 'regexp', args: '([\\d]+) times' }],
      },
      category: { text: 'Music' },
