...
export const definition: TrackerDefinition = {
  ...
  settings: [
    ...
    { name: 'password', type: 'password', label: 'Password' },
    {
      ...
      label: 'Layout',
      default:
        '<ol><li>Only the English Classic profile is supported.<li>Make sure to set the <b>Torrent Listing (Lista de Torrents)</b> option in your profile to <b>Classic (Clássico)</b><li>Using the <i>Moderna</i> theme will prevent results.</ol>',
    },
    {
      name: 'sort',
      type: 'select',
    ...
    },
    {
      ...
      default:
        "If you get 0 results check the log for error: <b>An error has occurred!You can't view this page unless you read all your unread messages</b>, then access the site with your browser and <b>mark as read</b> all PMs.",
    },
  ],
  login: {
    path: '?p=home&pid=1',
  ...
  search: {
    ...
    error: [
      {
        selector:
          'div.error:not(:contains("Não existem resultados encontrados.")):not(:contains("There are no results found."))',
      },
    ],
    rows: {
    ...
    fields: {
      ...
      imdb: {
        ...
        filters: [
          ...
          { name: 'prepend', args: 'https://www.imdb.com/title/' },
        ],
      },
      downloadvolumefactor: { case: { 'img[title="FREE!"]': 0, '*': 1 } },
      uploadvolumefactor: { text: 1 },
      date: {
        selector:
          'td.torrent_name:not(:has(abbr.timeago)):contains("day"), td.torrent_name:not(:has(abbr.timeago)):contains("Domingo"), td.torrent_name:not(:has(abbr.timeago)):contains("Sábado"), td.torrent_name:not(:has(abbr.timeago)):contains("Quinta"), td.torrent_name:not(:has(abbr.timeago)):contains("Sexta"), td.torrent_name:not(:has(abbr.timeago)):contains("Quarta"), td.torrent_name:not(:has(abbr.timeago)):contains("Terça"), td.torrent_name:not(:has(abbr.timeago)):contains("Segunda"), td.torrent_name:not(:has(abbr.timeago)):contains("ontem"), td.torrent_name:not(:has(abbr.timeago)):contains("Hoje")',
        optional: true,
        remove: 'span, a, br',
        filters: [
          { name: 'replace', args: ['Enviado ', ''] },
          { name: 'replace', args: ['Uploaded ', ''] },
          { name: 'replace', args: [' por', ''] },
          { name: 'replace', args: [' by', ''] },
          { name: 'replace', args: ['às ', ''] },
          { name: 'replace', args: ['at ', ''] },
          { name: 'replace', args: ['Hoje', 'Today'] },
          { name: 'replace', args: ['ontem', 'Yesterday'] },
          { name: 'replace', args: ['Segunda', 'Monday'] },
          { name: 'replace', args: ['Terça', 'Tuesday'] },
          { name: 'replace', args: ['Quarta', 'Wednesday'] },
          { name: 'replace', args: ['Quinta', 'Thursday'] },
          { name: 'replace', args: ['Sexta', 'Friday'] },
          { name: 'replace', args: ['Sábado', 'Saturday'] },
