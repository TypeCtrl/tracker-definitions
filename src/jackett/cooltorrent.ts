...
export const definition: TrackerDefinition = {
  ...
  type: 'semi-private',
  encoding: 'UTF-8',
  links: ['http://cool-torrent.pl/'],
  legacylinks: ['https://cool-torrent.pl/'],
  caps: {
    categorymappings: [
      ...
      {
        id: 'Filmy HD 1080p AVC',
        cat: 'Movies/HD',
        desc: 'Filmy HD 1080p AVC',
      },
  ...
  search: {
    paths: [{ path: 'szukaj.php' }],
    inputs: { 'form-display-time': 1, cat: 0, search: '{{ .Keywords }}' },
    rows: {
    ...
    fields: {
      ...
      size: {
        selector: 'td.descr3',
        filters: [{ name: 'regexp', args: '(\\d{1,4}\\.\\d{2}\\s[K|M|G][B])' }],
      },
      seeders: {
        selector: 'td.descr3',
        filters: [{ name: 'regexp', args: 'Seedów: (\\d+)' }],
      },
      leechers: {
        selector: 'td.descr3',
        filters: [{ name: 'regexp', args: 'Peerów: (\\d+)' }],
      },
      grabs: {
        selector: 'td.descr3',
        filters: [{ name: 'regexp', args: 'Pobrano: (\\d+)' }],
      },
